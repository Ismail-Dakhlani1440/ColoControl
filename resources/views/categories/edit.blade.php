...
<head>
    ...
    <title>Edit Category - ColoControll</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-card { background: rgba(255,255,255,0.95); backdrop-filter: blur(10px); border-radius: 2rem; padding: 2.5rem; box-shadow: 0 20px 40px rgba(0,0,0,0.1); width: 100%; max-width: 560px; margin-top: 2rem; border-left: 5px solid var(--cat-color, #ff6b6b); }
        ...
        .usage-badge { display: inline-flex; align-items: center; gap: 0.4rem; background: #f0fdf4; color: #15803d; border: 1px solid #bbf7d0; padding: 0.375rem 0.75rem; border-radius: 2rem; font-size: 0.8rem; font-weight: 600; margin-bottom: 1.5rem; }
    </style>
</head>
<body>
    ...
    <div class="dashboard-container">
        ...
        <main class="main-content">
            <div class="form-card" style="--cat-color: {{ $category->color }}">
                <h1 class="form-title">Edit Category</h1>
                <p class="form-subtitle">{{ $flatShare->name }}</p>

                @php $expenseCount = $category->expenses()->count(); @endphp
                @if($expenseCount > 0)
                    <div class="usage-badge">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <polyline points="20 6 9 17 4 12"/>
                        </svg>
                        Used in {{ $expenseCount }} {{ Str::plural('expense', $expenseCount) }}
                    </div>
                @endif
                ...
                <form action="{{ route('categories.update', $category) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <div class="color-row">
                            <input type="color" id="colorPicker" value="{{ old('color', $category->color) }}"
                                   class="color-input"
                            ...
                            <div id="colorPreview" class="color-preview" style="background: {{ old('color', $category->color) }}"></div>
                        </div>
                        @error('color')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
    ...
    <script>
        function syncColor(val) {
            ...
            document.querySelector('.form-card').style.setProperty('--cat-color', val);
        }
